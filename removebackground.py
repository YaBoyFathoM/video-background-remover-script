...
def remove_background(input_video,model_path = 'isnet-general-use.pth', output_folder='output'):
    ...
    while video.isOpened():
        ...
        with torch.no_grad():
            im_shp=frame.shape[0:2]
            im_tensor = torch.tensor(frame, dtype=torch.float32).permute(2,0,1)
            ...
            result_rgb = torch.cat((result, result, result), dim=0)
            frame = frame * result_rgb.cpu().data.numpy().transpose(1, 2, 0)
        frame_list.append(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB))
    video = ImageSequenceClip(frame_list, fps=30)
    if audio.duration > video.duration:
        audio = audio.subclip(0, video.duration)
    elif audio.duration < video.duration:
        audio = audio.fx(audio_normalize)
    final_video = video.set_audio(audio)
    final_video.write_videofile(input_video, codec='libx264', audio_codec='aac')
    frame_list.clear()
